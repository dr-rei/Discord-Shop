...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option
        .setName("nodes-id")
        .setDescription("Input the nodes ID")
        .setRequired(true)
    )
    .addIntegerOption((option) =>
      option
        .setName("total")
        .setDescription("Input Total Allocation That Will Generated")
        .setRequired(true)
    )
    .addIntegerOption((option) =>
      option
        .setName("min-range")
        .setDescription("Minimun range for allocation port")
        .setRequired(true)
    )
    .addIntegerOption((option) =>
      option
        .setName("max-range")
        .setDescription("Maximum range for allocation port")
        .setRequired(true)
    ),
  async execute(interaction, config) {
    ...
    if (!allowed) {
      ...
      for (var i = 0; i < allocationLimit; ) {
        ...
        let allocationCheck = await arrayAllocation.find(
          (registeredAllocation) =>
            registeredAllocation.attributes.port === randNumber
        );
        let allocation2Check = await portAllocation.find(
          (registeredAllocation) => registeredAllocation === randNumber
        );
        if (allocationCheck || allocation2Check) {
          console.log(
            "there is duplicate port from randomizer that port is " + randNumber
          );
        } else {
      ...
      const registerAllocation = await application.createNodeAllocations(
        nodeId,
        "0.0.0.0",
        portAllocation
      );
